...
import com.zby.fin.core.listener.ExcelDictDTOListener;
import com.zby.fin.core.mapper.DictMapper;
...
public class DictServiceImpl extends ServiceImpl<DictMapper, Dict> implements DictService {
    ...
    public void importData(InputStream inputStream) {
        ...
        //这里的baseMapper是被自动注入的dictMapper
    ...
    public List<Dict> listByParentId(Long parentId) {
        ...
        //先查询redis中是否存在数据列表
        try {
            dictList = (List<Dict>)redisTemplate.opsForValue().get("fin:core:dictList:" + parentId);
            if(dictList != null){
                log.info("从redis中取值");
                return dictList;
            }
        ...
        try {
            //5分钟过期
            redisTemplate.opsForValue().set("fin:core:dictList:" + parentId, dictList, 5, TimeUnit.MINUTES);
            log.info("数据存入redis");
        } catch (Exception e) {
            log.error("redis服务器异常：" + ExceptionUtils.getStackTrace(e));//此处不抛出异常，继续执行后面的代码
        }
